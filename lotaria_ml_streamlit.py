...
def secao_adicionar_resultado(analyzer: AnalisadorLotaria) -> None:
    ...
    if foto is not None:
        ...
        # Validação final: 1-90, sem duplicados (já filtrado no OCR, mas
        # garante consistência com RepositorioResultados.validar_resultado).
        extraidos = [
            n for n in extraidos
        ...
        if len(extraidos) >= 5:
            ...
            try:
                RepositorioResultados.validar_resultado(sorteio_input, valores_default)
                st.sidebar.success(f"OCR encontrou: {', '.join(map(str, valores_default))}")
            except ResultadoInvalidoError as err:
                st.sidebar.warning(f"OCR leu números inválidos ({err}). Confirma manualmente.")
                valores_default = [i + 1 for i in range(len(COLUNAS_NUMEROS))]
        elif extraidos:
...
def pagina_verificar_numeros(analyzer: AnalisadorLotaria) -> None:
    ...
    with st.expander("Calculadora de probabilidade (hipergeométrica)", expanded=False):
        st.caption(
            "Probabilidade exacta de acertar pelo menos K números, "
            "só com combinatória — sem ML. Útil para decidir quantos "
            "números jogar se o objectivo for garantir chance de ≥2 acertos."
        )
        col_a, col_b = st.columns(2)
        with col_a:
            k_jogados = st.slider("Quantos números jogas", 5, 30, 5, key="hyp_k_jog")
        with col_b:
            k_alvo = st.slider("Acertos mínimos desejados", 1, 5, 2, key="hyp_k_alvo")
        p = prob_pelo_menos_k_acertos(k_jogados, k_alvo)
        st.metric(
            f"P(acertar ≥ {k_alvo} com {k_jogados} números)",
            f"{p * 100:.2f}%",
        )
        # tabela rápida de referência
        ref_rows = []
        for kj in (5, 8, 10, 12, 15, 20):
            ref_rows.append({
                "Números jogados": kj,
                "P(≥1)": f"{prob_pelo_menos_k_acertos(kj, 1)*100:.1f}%",
                "P(≥2)": f"{prob_pelo_menos_k_acertos(kj, 2)*100:.1f}%",
                "P(≥3)": f"{prob_pelo_menos_k_acertos(kj, 3)*100:.1f}%",
            })
...
def pagina_historico_sugestoes(analyzer: AnalisadorLotaria) -> None:
    ...
    baseline = stats.get("baseline_aleatoria") or {}
    c1, c2, c3, c4 = st.columns(4)
    with c1:
    ...
    with c4:
        base_m = baseline.get("media_melhor")
        st.metric(
            "Baseline aleatória (melhor)",
            f"{base_m:.2f}" if base_m is not None else "—",
            help=(
                "Média do melhor acerto se as 4 opções fossem números "
                "aleatórios. Se o modelo ≈ baseline, não há sinal real "
                "a aprender (esperado em sorteios independentes)."
            ),
        )
    if media is not None and base_m is not None:
        diff = media - base_m
        if abs(diff) < 0.15:
            st.caption(
                f"Modelo ≈ acaso (diferença {diff:+.2f}). Em sorteios independentes "
                "isto é o resultado honesto esperado — o modelo não está a 'falhar'."
            )
        elif diff > 0:
            st.caption(f"Modelo acima do acaso em {diff:+.2f} acertos (média).")
        else:
            st.caption(f"Modelo abaixo do acaso em {diff:+.2f} acertos (média).")
