...
class Config:
    ...
    # Limite de sorteios usados no treino do RF. Mais dados aqui não
    # melhoram a previsão (sorteios independentes) e só aumentam tempo
    # e memória — crítico no Streamlit Cloud gratuito.
    max_sorteios_treino: int = 2500
...
def _binom(n: int, k: int) -> float:
    """C(n, k) em float (evita overflow de int para n grandes)."""
    if k < 0 or k > n:
        return 0.0
    k = min(k, n - k)
    res = 1.0
...
def prob_pelo_menos_k_acertos(
    k_jogados: int,
    k_alvo: int = 2,
    total: int = NUMERO_MAXIMO,
    sorteados: int = QTD_NUMEROS_POR_SORTEIO,
) -> float:
    """P(acertar >= k_alvo) jogando k_jogados números de `total`.

    Distribuição hipergeométrica exacta:
      população = total, "sucessos" na população = sorteados,
      amostra = k_jogados.
    Não depende de ML — só de combinatória. Útil para mostrar ao
    utilizador que subir de 5 para 12-15 números aumenta bastante
    a chance de acertar ≥2, independentemente de qualquer previsão.
    """
    if k_jogados < 0 or k_alvo < 0:
        return 0.0
    if k_jogados > total:
        k_jogados = total
    # P(X >= k_alvo) = 1 - P(X <= k_alvo-1)
    denom = _binom(total, k_jogados)
    if denom == 0:
        return 0.0
    cdf = 0.0
    for k in range(0, k_alvo):
        # hipergeométrica: C(sorteados, k) * C(total-sorteados, k_jogados-k) / C(total, k_jogados)
        cdf += _binom(sorteados, k) * _binom(total - sorteados, k_jogados - k)
    return max(0.0, min(1.0, 1.0 - cdf / denom))
...
class PreditorLotaria:
    ...
    def _construir_dataset(self, df_local: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
        ...
        Se o histórico for maior que max_sorteios_treino, uso só a
        janela mais recente — mais dados não melhoram a previsão e
        só tornam o treino lento/pesado em memória.
        """
        n_disponiveis = len(df_local)
        if n_disponiveis > self.config.max_sorteios_treino:
            df_local = df_local.iloc[-self.config.max_sorteios_treino:].reset_index(drop=True)
            logger.info(
                "Treino limitado aos últimos %d sorteios (de %d disponíveis).",
                self.config.max_sorteios_treino, n_disponiveis,
            )
...
class RepositorioSupabase:
    ...
    Nota: o PostgREST devolve no máximo 1000 linhas por pedido.
    Por isso pagino com .range() e verifico o total com count="exact".
    """

    TABELA = "resultados"
    # Tamanho de página alinhado com o limite por omissão do PostgREST.
    TAMANHO_PAGINA = 1000
    ...
    def __init__(self, url: str, key: str):
        ...
        # Cache em memória do DataFrame: evita reler milhares de linhas
        # em cada interação. Invalidado em qualquer escrita.
        self._df_cache: Optional[pd.DataFrame] = None

    def invalidar_cache(self) -> None:
        """Força a próxima leitura de df a ir ao Supabase."""
        self._df_cache = None

    def _contar_linhas(self) -> Optional[int]:
        """Devolve o total exacto de linhas na tabela, ou None se falhar."""
        try:
            resp = (
            ...
            logger.exception("Falha ao obter count exacto de %s.", self.TABELA)
            return None

    def _carregar_paginado(self) -> list[dict]:
        """Lê todas as linhas com .range(), página a página.

        Continua enquanto a página vier cheia (TAMANHO_PAGINA).
        No fim compara o total lido com count="exact" e avisa se
        houver discrepância (protege contra off-by-one silencioso).
        """
        todas: list[dict] = []
        inicio = 0
        while True:
            ...
            try:
                ...
                logger.exception(
                    "Falha ao ler página %d-%d de %s.", inicio, fim, self.TABELA
                )
                raise
            pagina = resp.data or []
            todas.extend(pagina)
        ...
        total_esperado = self._contar_linhas()
        if total_esperado is not None and total_esperado != len(todas):
            logger.warning(
                "Discrepância na leitura de %s: li %d linhas, "
                "mas count exacto reporta %d. Possível off-by-one "
                "ou alteração concorrente durante a paginação.",
                self.TABELA, len(todas), total_esperado,
            )
        else:
            logger.info(
                "Lidas %d linhas de %s (paginação OK).",
                len(todas), self.TABELA,
            )
        return todas
    ...
    def df(self) -> pd.DataFrame:
        ...
        linhas = self._carregar_paginado()
        if not linhas:
            df = pd.DataFrame(columns=["Data", "Sorteio", *COLUNAS_NUMEROS])
        else:
    ...
    def importar_dataframe(self, df_novo: pd.DataFrame) -> int:
        ...
        for _, row in df_novo.iterrows():
            try:
                data = pd.to_datetime(row["Data"]).strftime("%Y-%m-%d")
                nums = [int(row[c]) for c in COLUNAS_NUMEROS]
                # Chamo a lógica de gravação sem invalidar a cada linha;
                # invalido uma vez no fim.
                RepositorioResultados.validar_resultado(str(row["Sorteio"]), nums)
                registro = {
                    "data": data,
                    "sorteio": str(row["Sorteio"]),
                    **{f"n{i + 1}": int(n) for i, n in enumerate(nums)},
                }
                self._client.table(self.TABELA).upsert(
                    registro, on_conflict="data,sorteio"
                ).execute()
                ...
                logger.exception("Falha ao gravar linha no import Supabase.")
        self.invalidar_cache()
        return gravados
...
class ArmazenamentoSugestoesSupabase:
    ...
    def carregar_tudo(self) -> dict:
        ...
        Pagina com .range() porque o PostgREST limita a 1000 linhas
        por pedido. Compara o total lido com count="exact".
        """
        todas: list[dict] = []
        inicio = 0
        try:
...
class AnalisadorLotaria:
    ...
    def baseline_aleatoria(
        ...
        """Simula sugestões aleatórias no mesmo número de sorteios.

        Para cada sorteio simulado, gera `n_opcoes` grupos de 5 números
        aleatórios e compara com um resultado real também aleatório
        (hipótese de independência). Devolve a média do melhor acerto
        e a média de todas as opções — o que um modelo honesto deve
        aproximar se não houver sinal real no histórico.
        """
        if n_sorteios <= 0:
            return {"media_melhor": None, "media_todas_opcoes": None, "n_simulacoes": 0}
        ...
        for _ in range(n_simulacoes):
            ...
            for _ in range(n_sorteios):
                real = set(rng.choice(numeros, size=QTD_NUMEROS_POR_SORTEIO, replace=False).tolist())
                acertos_op = []
                for _ in range(n_opcoes):
                    op = set(rng.choice(numeros, size=QTD_NUMEROS_POR_SORTEIO, replace=False).tolist())
                    a = len(real & op)
                    acertos_op.append(a)
                    todas.append(a)
                somas_melhor += max(acertos_op)
            melhores.append(somas_melhor / n_sorteios)

        return {
            "media_melhor": float(np.mean(melhores)),
            "media_todas_opcoes": float(np.mean(todas)) if todas else None,
    ...
    def estatisticas_acertos(self) -> dict:
        ...
        Inclui comparação com baseline aleatória: se media_melhor ≈ baseline,
        o modelo não está a aprender sinal real (esperado em sorteios independentes).
        """
        ...
        n_opcoes = len(com_resultado[0]["acertos"]) if com_resultado[0]["acertos"] else 4
        ...
        baseline = self.baseline_aleatoria(
